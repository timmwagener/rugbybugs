...
#rbAutoChrystalRenderImportFunctionality module
#------------------------------------------------------------------
'''
...
#RbAutoChrystalRenderImportFunctionality class
#------------------------------------------------------------------
class RbAutoChrystalRenderImportFunctionality():
	'''RbAutoChrystalRenderImportFunctionality class'''
	...
	def unlinkChrystalFromLights(self):
		...
		#recreate lightlinks
		pm.lightlink( m = True, light = intermediary_lights_lightlink_nodeList, object=(self.innerHull_grp, self.intermediary_grp))
		pm.select(cl = True)
	...
	def assignShaderOnRLayer(self):
		...
		pm.select(cl = True)
		#rockremainder
		self.shaderOverrideOnRLayer(rlayer = self.chrystal_shader_intermediary_mask_rlyr, shader = self.intermediary_sss_mask_rockremainder_main_mt, geometryList = [self.inner_chrystal_rockremainder_grp])
		pm.select(cl = True)
