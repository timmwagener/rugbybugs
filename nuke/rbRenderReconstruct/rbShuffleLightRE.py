...
#RbShuffleLightRE class
#------------------------------------------------------------------

class RbShuffleLightRE():
	...
	def reconstructLightRenderElements(self, node):
		...
		if not(self.getLayers(node, 'rbDiffuse')):
		...
		layerListRbLs = self.getLayers(node, 'rbLs')
			
		#get rbGi
		layerListRbGi = self.getLayers(node, 'rbGi')
			
		#Merge GI and LightSelects
		layerListLight = layerListRbLs + layerListRbGi
		...
		if(len(layerListLight) > 0):
			...
			for layer in layerListLight:
				...
				if(layer == 'rbGi'): outputDotList.append(self.reconstructLightRE(node, layer, gi = True))
			...
			if(self.getLayers(node, 'rbSelfIllum')): outputDotList.append(self.shuffleRE(node, 'rbSelfIllum', frameBuffer = True))
	...
	def reconstructLightRE(self, node, layer, gi = False):
		...
		#shuffleDiffuse
		shuffleDiffuse = nuke.nodes.Shuffle(label = 'rbDiffuse_rgb', inputs = [node])
		shuffleDiffuse['in'].setValue( 'rbDiffuse' )
		...
		if(gi == True):
			...
			#get rbEtAmbOcc
			layerListAmbOcc = self.getLayers(node, 'rbEtAmbOcc')
			...
			else: print('No rbEtAmbOcc layer exists')
		...
		if(connectAmbOcc): outputDot.setInput(0, multiplyGiWithAmbOcc)
		else: outputDot.setInput(0, multiplyRawLightWithDiffuse)
